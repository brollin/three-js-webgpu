...
function init() {
  ...
  const computeInit = tslFn(() => {
    ...
    position.y = randY.mul(50);
    position.z = randZ.mul(100).add(-50);

    position.x = 0;
    position.z = 0;

    velocity.x = randX.sub(0.5).mul(1);
    velocity.z = randZ.sub(0.5).mul(1);
  ...
  gui.add(size, "value", 0.18, 0.5, 0.01).name("size");
}
